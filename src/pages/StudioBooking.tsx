...
export default function StudioBooking() {
  ...
  const loadCal = useCallback(async () => {
    ...
    setCalEvents(
      (data ?? []).map(row => ({
        ...
        textColor:       '#ffffff',
        extendedProps:   { status: row.status, type: row.booking_type },
      }))
    )
  }, [isAdmin])
  ...
  async function handleSubmit() {
    ...
    if (!bandName.trim())  return setFormError('Band / artist name is required.')
    if (!bookingDate)      return setFormError('Booking date is required.')
    if (!startTime)        return setFormError('Start time is required.')
    if (!endTime)          return setFormError('End time is required.')
    ...
    if (error) {
      ...
      setFormSuccess("Session request submitted! We'll confirm via email shortly.")
    ...
    <div className="booking-page">
        ...
        <div className="booking-card">
          ...
          <div className="bk-field">
            <IonInput
              ...
              fill="outline"
              value={bandName}
              onIonInput={e => setBandName(e.detail.value ?? '')}
              placeholder="e.g. The Northside Band, Juan Dela Cruz"
              className="bk-input"
            />
          </div>
          ...
          <IonItem lines="none" className="studio-hours-note-item">
            <IonNote>
              Studio hours: Tue–Thu 9am–10pm · Fri–Sat 9am–11pm · Sun 10am–9pm
            </IonNote>
          </IonItem>
      ...
      <section className="booking-section">
        <div className="booking-section-header">
          ...
          <h2 className="bsh-title">
            {isAdmin ? 'All Studio Sessions' : 'Studio Availability'}
          </h2>
        </div>
        {!isAdmin && (
          <p className="cal-note">
            Booked time slots are shown. Choose an available date and time above.
          </p>
        )}
      ...
      <section className="booking-section" style={{ paddingBottom: 32 }}>
        ...
        {loadingList ? (
          ...
          <div className="bk-empty">
            <IonIcon icon={musicalNotesOutline} />
            <p>No session requests yet. Fill in the form above to book a slot!</p>
          </div>
        ) : (
